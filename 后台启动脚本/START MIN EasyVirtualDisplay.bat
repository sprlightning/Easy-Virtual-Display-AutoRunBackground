...
set TITLE=START_MIN_EasyVirtualDisplay
...
echo [%DATE%%TIME:~0,8%]%executable% is running!
echo:
echo [%DATE%%TIME:~0,8%]%executable% is running! >> %TITLE%.log
echo: >> %TITLE%.log

ping 127.0.0.1 -n 1 > nul

msg %username% /time:6 "已开启虚拟屏%executable%！"
